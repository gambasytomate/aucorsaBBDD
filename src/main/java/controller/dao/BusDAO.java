...
public class BusDAO {
     ...
     */
    public static Bus buscarBus(Connection con, String register) {

        // Sentencia SQL parametrizada para buscar un bus por su matrícula
        String sql = "SELECT * FROM Bus WHERE register = ?;";
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                if (rs.next()) {
                    String matriculaBus = rs.getString("register");
                    String tipo = rs.getString("type");
                    String licencia = rs.getString("license");
                    String imagen = rs.getString("imagen");
                    return new Bus(matriculaBus, tipo, licencia, imagen);
                }
            }
        } catch (SQLException e) {
            // Imprime información de error si ocurre una excepción SQL
    ...
    public static List<Bus> mostrarTodosLosBuses(Connection con) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                String licencia = rs.getString("license");
                String imagen = rs.getString("imagen");
                buses.add(new Bus(matricula, tipo, licencia, imagen));
            }
    ...
    public static boolean modificarBus(Connection con, Bus bus) {
        ...
        String sql = "UPDATE Bus SET type = ?, license = ? WHERE register = ?";
