...
import controller.DetailsBusController;
...
public class BusController {
     ...
     */
    public BusController(BusPanel panel, BusDAO busDAO) {
        this.busPanel = panel;
        this.busDAO = busDAO;

        busPanel.getTablaVista().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && busPanel.getTablaVista().getSelectedRow() != -1) {
                    view.bus.DetailsBusView vista = new DetailsBusView();
                    new DetailsBusController(vista,BusController.this, busPanel, buses, busDAO);
                }
            }
        });
        cargarBuses();
